...
def retrieve_DB(state):
    ...
    Returns:
        state (dict): New key may be added to state, generation, 
        which contains the answer for query asked
    """
    ...
    document_dict = dict()
    retrieved_dict = query_retriever.invoke(
        {"query": query, "chat_history": [], "agent_scratchpad": []}
    )
    document_dict["mongodb_query"] = retrieved_dict["intermediate_steps"][0][
        0
    ].tool_input["agg_pipeline"]
    document_dict["retrieved_output"] = retrieved_dict["intermediate_steps"][
        0
    ][1]
...
def filter_generator(state):
    ...
    Returns:
        state (dict): New key may be added to states, filter and top_k, 
        which contains the MongoDB query that will be applied before retrieval
    """
